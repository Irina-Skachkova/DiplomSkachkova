...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//++ i.skachkova 
	Если Элементы.Найти("ГрафикРаботы") = Неопределено Тогда
		НовыйЭлемент = Элементы.Добавить("ГрафикРаботы", Тип("ПолеФормы"), Элементы.ГруппаДанныеШапки);		
		НовыйЭлемент.ПутьКДанным = "Объект.ВКМ_ГрафикРаботы";
		НовыйЭлемент.Вид = ВидПоляФормы.ПолеВвода;
		НовыйЭлемент.Заголовок = "График работы";
	КонецЕсли; 
...
//++ i.skachkova 15.05.2024
&НаКлиентеНаСервереБезКонтекста 
Процедура ВывестиЭлементыНаФорму(Форма)
	
	Форма.Элементы.ГрафикРаботы.Видимость = Истина;	
